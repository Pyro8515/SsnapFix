-- Migration: Improve requirements union logic to properly de-dupe
-- When a requirement exists both globally and trade-specifically, check it only once
-- Trade-specific requirements take precedence over global requirements
...
BEGIN
    ...
    LOOP
        ...
        -- De-dupe: If same requirement exists both globally and trade-specifically, use trade-specific
        all_compliant := true;
        missing_docs := ARRAY[]::TEXT[];

        -- Check requirements: union global + trade-specific, de-dupe by (doc_type, doc_subtype)
        -- Trade-specific requirements take precedence (ORDER BY trade NULLS LAST)
        FOR required_docs IN
            SELECT DISTINCT ON (doc_type, COALESCE(doc_subtype, ''))
                doc_type, doc_subtype, is_critical, trade
            FROM trade_requirements
            WHERE (trade IS NULL OR trade = trade_item)
            AND is_required = true
            ORDER BY doc_type, COALESCE(doc_subtype, ''), trade NULLS LAST
        LOOP
            ...
            IF NOT FOUND OR user_docs IS NULL THEN
                all_compliant := false;
                -- Only add to missing_docs if not already present (de-dupe)
                IF NOT (required_docs.doc_type || 
                    CASE WHEN required_docs.doc_subtype IS NOT NULL THEN ' (' || required_docs.doc_subtype || ')' ELSE '' END
                    = ANY(missing_docs)) THEN
        ...
        IF EXISTS (
            ...
            IF NOT EXISTS (
                ...
                all_compliant := false;
                -- Only add if not already in missing_docs
                IF NOT ('government_id (Stripe Identity verification)' = ANY(missing_docs)) THEN
